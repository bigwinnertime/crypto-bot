...
def test_trailing_stop_strategy():
    ...
    # 模拟配置（避免依赖config.py的dotenv）
    test_config = {
        'ETH/USDT': {
            'trailing_stops': [
                {'profit_threshold': 0.03, 'trailing_pct': 0.02},   # 3%盈利后，2%回撤止损
                {'profit_threshold': 0.06, 'trailing_pct': 0.025},  # 6%盈利后，2.5%回撤止损
                {'profit_threshold': 0.12, 'trailing_pct': 0.03},   # 12%盈利后，3%回撤止损
            ],
            'time_decay': {
                'enabled': True,
                'intervals': [
    ...
    for price, desc in price_sequence:
        ...
        if stop_reason:
            ...
            trailing_pct = status['adjusted_trailing_pct']
            trailing_str = f"{trailing_pct:.2%}" if trailing_pct is not None else "N/A"
            print(f"  继续持有 (追踪比例: {trailing_str})")
